...
dumpI2CData("dump write in I2C_Write", slave_addr, buffer, numberOfBytes);
...
uint16_t defaultManualSpeed[] = { 25, 25, 17, 7, 600, 800 };
...
do {
	...
	parse_param(buf, "MANUAL_SPEED", &defaultManualSpeed[1], 5);
...
int MSP430FR2311_Set_AutoModeWithAngle(int mode, int angle) {
   ...
   if (mode==1 || mode==2)   {
	...
	if (mode==5 || mode==6) { 
		 uint16_t AutoEmergency2Mode[]={0, 600, 600, 900, 900, 600, 600, 30, 60, 100, 280, 350, 420};
		AutoEmergency2Mode[0]=mode-5;
		powerDownDuration=DEFAULT_POWERDOWNDURATION;
		return MSP430FR2311_Set_ParamMode(AutoEmergency2Mode);
	}
