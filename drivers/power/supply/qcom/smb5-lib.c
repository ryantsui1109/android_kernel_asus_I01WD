...
const u32 default_src_caps[] = { 0x360190c8 };  /* VSafe5V @ 2A */
...
static bool batt_temp_alert = false;
static bool set_current_drop = false;
...
int smblib_vbus_regulator_disable(struct regulator_dev *rdev)
{
	...
	batt_temp_alert = false;
	set_current_drop = false;
...
int asus_get_batt_status (void)
{
	if(smbchg_dev->pd_active){
		if(final_pd_mw > 10000)
			return QC_PLUS;
		else if(final_pd_mw == 10000)
			return QC;
		else
			return NORMAL;
	} 
...
void asus_reverse_charge_work(struct work_struct *work)
{
	...
	batt_temp = asus_get_prop_batt_temp(smbchg_dev);
	batt_current = asus_get_prop_batt_current(smbchg_dev);
	batt_capacity = asus_get_prop_batt_capacity(smbchg_dev);
	
	if (batt_temp > 550) {
		CHG_DBG("batt_temp > 55C, disable OTG\n");
		//0x1140[0] = 0, Disable OTG
		rc = smblib_masked_write(smbchg_dev, DCDC_CMD_OTG_REG, OTG_EN_BIT, 0);
		if (rc < 0) {
			CHG_DBG_E("Couldn't disable OTG regulator rc=%d\n", rc);
		}
		batt_temp_alert = true;
	} 
	else if (batt_temp_alert && batt_temp < 52) {
		CHG_DBG("batt_temp < 52C, enable OTG\n");
		//0x1140[0] = 1, Enable OTG
		rc = smblib_masked_write(smbchg_dev, DCDC_CMD_OTG_REG, OTG_EN_BIT, 1);
		if (rc < 0) {
			CHG_DBG_E("Couldn't enable OTG regulator rc=%d\n", rc);
		}
		batt_temp_alert = false;
	}
	
	if (!batt_temp_alert && !set_current_drop && (batt_current > 3500000 || batt_capacity <= 20)) {
		CHG_DBG("battery current > 3.5A or battery capacity <= 20\n");
		//0x1140[0] = 0, Disable OTG
		rc = smblib_masked_write(smbchg_dev, DCDC_CMD_OTG_REG, OTG_EN_BIT, 0);
		if (rc < 0) {
			CHG_DBG_E("Couldn't disable OTG regulator rc=%d\n", rc);
		...
		//0x1140[0] = 1, Enable OTG
		rc = smblib_masked_write(smbchg_dev, DCDC_CMD_OTG_REG, OTG_EN_BIT, OTG_EN_BIT);
		if (rc < 0) {
			CHG_DBG_E("Couldn't enable OTG rc=%d\n", rc);
		}
		
		set_current_drop = true;
	}
	
	schedule_delayed_work(&smbchg_dev->asus_reverse_charge_work, msecs_to_jiffies(5000));
}
...
void asus_usb_thermal_work(struct work_struct *work)
{
	...
	if (usb_thermal_once_flag && !usb_thermal_unplug_flag) {
		CHG_DBG("[usb_thermal] conn_temp(%d) keep suspended by flag, usb(%d), otg(%d), usb_connector = %d\n",
			conn_temp, usb_present, otg_present, asus_extcon_get_state(smbchg_dev->thermal_extcon));
	} else if (conn_temp >= 700) {
		if (usb_present) {
			if (!otg_present) {
		...
		usb_thermal_unplug_flag = false;
		CHG_DBG("[usb_thermal] conn_temp(%d) >= 700, usb(%d), otg(%d), usb_connector = %d\n",
			conn_temp, usb_present, otg_present, asus_extcon_get_state(smbchg_dev->thermal_extcon));
	} else if (conn_temp <= 600) {
		if (usb_thermal_unplug_flag) {
			usb_thermal_once_flag = false;
			usb_thermal_unplug_flag = false;
			smblib_set_usb_suspend(smbchg_dev, 0);
			CHG_DBG("[usb_thermal] Restore usb current, unplug and <= 500\n");
		}
		asus_extcon_set_state_sync(chg->thermal_extcon, THERMAL_ALERT_NONE);
		CHG_DBG("[usb_thermal] conn_temp(%d) <= 600, usb(%d), otg(%d), usb_connector = %d\n",
			conn_temp, usb_present, otg_present, asus_extcon_get_state(smbchg_dev->thermal_extcon));
	} else {
		if (usb_thermal_unplug_flag) {
			usb_thermal_unplug_flag = false;
			CHG_DBG("[usb_thermal] Keep usb current suspended, unplug but > 600\n");
		}
		CHG_DBG("[usb_thermal] 600 < conn_temp(%d) < 700, usb(%d), otg(%d), usb_connector = %d\n",
			conn_temp, usb_present, otg_present, asus_extcon_get_state(smbchg_dev->thermal_extcon));
	}
