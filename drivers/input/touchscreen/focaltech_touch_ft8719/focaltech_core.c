...
void pmode_tp_en(bool pmode_en)
{
	int retval;
	if (pmode_en) {
		printk("[FTS][touch] %s:FTS_REG_PMODE enable\n", __func__);
		retval = fts_i2c_write_reg(fts_data->client, 0xC8, 0x01);
		if (retval < 0)
			FTS_ERROR("set TP enable pmode fail, ret=%d", retval);
	} else {
		printk("[FTS][touch] %s:FTS_REG_PMODE disable\n", __func__);
		retval = fts_i2c_write_reg(fts_data->client, 0xC8, 0x00);
		if (retval < 0)
			FTS_ERROR("set TP disable pmode fail, ret=%d", retval);
	}
}
/*****************************************************************************
*  Name: fts_ts_resume
...
void fts_set_pmode(bool pmode)
{
	if (pmode) {
		pmode_tp_flag = true;
		//printk("fts_set_pmode pmode_tp_flag = true\n");
		pmode_tp_en(true);
	} else {
		pmode_tp_flag = false;
		//printk("fts_set_pmodepmode_tp_flag = false\n");
		pmode_tp_en(false);
	}
}
