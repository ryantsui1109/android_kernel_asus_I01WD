...
static void icm_acc_data_process(struct icm_sensor *sensor);
 ...
 */
static void icm_read_accel_data(struct icm_sensor *sensor,
			     struct axis_data *data)
{
	u16 buffer[3];

	icm_read_reg(sensor, sensor->reg.raw_accel,
 ...
 */
static void icm_read_gyro_data(struct icm_sensor *sensor,
			     struct axis_data *data)
{
	u16 buffer[3];

	icm_read_reg(sensor, sensor->reg.raw_gyro,
 ...
 */
static void icm_read_single_event(struct icm_sensor *sensor)
{
	u32 shift;
	...
	if (sensor->cfg.gyro_enable && icm_is_gyro_data_ready()) {
...
static irqreturn_t icm_interrupt_thread(int irq, void *data)
{
	...
	/*icm_dbgmsg("Interrupt source=0x%x\n", ret);*/

	dev_dbg(sensor->dev, "Interrupt source=0x%x\n", ret);

	if (ret & BIT_FIFO_OVERFLOW)
		icm_flush_fifo(sensor);
	else if (ret & (BIT_WOM_Z_INT | BIT_WOM_Y_INT | BIT_WOM_X_INT))
		icm_read_single_event(sensor);
	else if (ret & BIT_DATA_RDY_INT)
		icm_read_single_event(sensor);
	else
		dev_info(sensor->dev, "Unknown interrupt 0x%x", ret);
...
{
	icm_read_accel_data(sensor, &sensor->axis);
	/*icm_dbgmsg("before remap - x = %d, y = %d, z = %d\n", sensor->axis.x, sensor->axis.y, sensor->axis.z);*/
	icm_remap_accel_data(&sensor->axis, sensor->pdata->place);
	/*icm_dbgmsg("after remap - x = %d, y = %d, z = %d\n", sensor->axis.x, sensor->axis.y, sensor->axis.z);*/
